...
Кожне значення в JavaScript має тип. Цікаво, а який тип у функції?
...
Що доволі кумедно, логіка присвоєння "name" досить розумна. Вона працює так, що призначає правильне ім’я функції, навіть якщо функція була створена без імені, а потім була негайно призначена:
...
У специфікації ця ознака називається "контекстне ім’я" (contextual name). Якщо функція не надає власне ім’я, то в присвоєнні воно береться з контексту.
...
Але на практиці більшість функцій мають назву.
...
Після того, як користувач надає відповідь, функція викликає обробників. Ми можемо передати два типи обробників:

- функція без аргументів, яка викликається тільки коли користувач дає позитивну відповідь.
- функція з аргументами, яка викликається в будь-якому випадку, і повертає відповідь.
...
function sayHi() {
  ...
  *!*
  // давайте порахуємо, скільки викликів функції ми зробили
  sayHi.counter++;
  */!*
}
...
```warn header="Властивість - це не змінна"
...
В перекладі Іменований Функціональний Вираз.

Named Function Expression, або NFE -- це термін для Function Expressions, у якого є назва.

Наприклад, оголосімо звичайний Function Expression:
...
Спочатку відмітимо, що у нас ще є Function Expression. Додавання назви `"func"` після `function` не робить оголошення функції у вигляді Functional Declaration, оскільки функція все ще є частиною виразу присвоєння.
...
Наприклад, функція `sayHi` нижче викликає себе знову з `"Гість"` якщо `who` не надається:
...
Чому ми використовуємо `func`? Можливо краще просто використовувати `sayHi` для вкладеного виклику?


І справді в більшості випадків ми можемо це зробити:
...
Це відбувається тому, що функція бере `sayHi` з свого зовнішнього лексичного середовища. Там немає місцевого `sayHi`, тому використовується зовнішня змінна. І в момент виклику значення зовнішньої `sayHi` є `null`.
...
Тепер це працює, бо назва `"func"` -- локальна і знаходиться в середині функції. Вона не береться ззовні (і не доступна звідти). Специфікація гарантує, що вона завжди посилається на поточну функцію.
...
- `name` -- назва функції. Зазвичай береться з оголошення функції. Але якщо при оголошенні явно не було вказано, то JavaScript намагається здогадатися з контексту (наприклад, з присвоєння).
- `length` -- кількість аргументів в оголошенні функції. Але параметри, що зібрані за допомогою rest оператора, не підраховуються.
...
Також функції можуть зберігати додаткові властивості. Багато відомих бібліотек JavaScript активно використовують цю властивість функції.

Вони створюють "головну" функцію і додають багато інших "допоміжних" функцій до неї. Наприклад, бібліотека [jQuery](https://jquery.com) створює функцію, що називається `$`. Бібліотека [lodash](https://lodash.com) створює функцію `_`, а потім додає `_.clone`, `_.keyBy` та інші властивості до неї (див. [документацію](https://lodash.com/docs), щоб дізнатися більше). Власне, вони роблять це, щоб зменшити своє забруднення глобального простору імен, так що одна бібліотека дає лише одну глобальну змінну. Це зменшує можливість конфліктів імен.


Отже, функція може робити корисну роботу сама по собі, а також зберігати купу інших функцій у своїх властивостях.
